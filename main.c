...
#include <unistd.h>  /* fork, access, getcwd, chdir */
  ...
  do
    {
      ...
      home = getenv("HOME");
      if (home != NULL && starts_with(cwd, home))
        {
      ...
      if (strcmp(command_line[0], "exit") == 0)
        {
          exit_status = 0;
          /** If an exit status was provided as the second argument, use
              that. */
          if (argc > 1)
            {
      ...
      /** Fork this process into two processes. In each of the two processes,
          pid is set to the pid of that process. */
      pid = fork();
      ...
      /** If we are in the child process... */
      else if (pid == 0)
        {
          ...
          if (access_status == 0)
            ...
            {
              /** If the file exists but is not executable, don't search for
                  it. */
              if (errno == EACCES)
                {
              ...
              /** If the file doesn't exist, iterate over each directory in
                  PATH, if PATH is not empty. */
                  ...
                  for (i = 0; i < path_length; i++)
                    {
                      ...
                      if (access_status == 0)
                        {
                          /** Store the path to the program in
                              program_location. */
                          strncpy(program_location, temp_filename,
                        ...
                        {
                          /** If the file exists but is not executable, halt
                              the search. */
                          if (errno == EACCES)
                            {
                              perror("File is not executable");
              ...
              if (exec_result < 0)
                {
                  ...
                  /** The child process should terminate because no program was
                      run. */
      ...
      /** If we are in the parent process... */
        ...
        {
          /** Wait until the child process terminates and store its exit
              status. */
          wait(&child_status);
        }
    }
